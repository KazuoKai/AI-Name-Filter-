...
async function fetchWithTimeout(url, options, timeoutMs) {
  // Lỗi 9: Kết hợp timeout signal + global cancel signal từ nút Hủy
  const timeoutController = new AbortController();
  const id = setTimeout(() => timeoutController.abort(), timeoutMs);

  // Gộp signal timeout và signal hủy toàn cục (nếu có)
  const globalSignal = (typeof window !== 'undefined') ? window.__extractAbortSignal : null;
  let signal = timeoutController.signal;
  if (globalSignal && !globalSignal.aborted) {
    // Tạo signal kết hợp: abort nếu 1 trong 2 bị abort
    const mergedController = new AbortController();
    const onGlobalAbort = () => mergedController.abort();
...
// Cấu hình AI qua trung gian (OpenAI-compatible) — mặc định DeepSeek V4 Flash
let proxyBaseUrl = '';
let proxyModel = 'deepseek-v4-flash';
let proxyApiKey = '';
    ...
    proxyConfigPromise = (async () => {
      try {
        ...
        if (data && data.success) {
          ...
          if (data.model) proxyModel = String(data.model);
          if (data.apiKey) proxyApiKey = String(data.apiKey);
        }
      } catch (e) {
        // Chạy kiểu file:// (Electron) không có server config — dùng mặc định
      }
      if (!proxyBaseUrl) proxyBaseUrl = 'https://api.nexusmmo.store/v1';
      return { baseUrl: proxyBaseUrl, model: proxyModel, apiKey: proxyApiKey };
    })();
  }
